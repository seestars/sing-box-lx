...
| Статус | D (done) — red/green на юните (три вида обёрток) и e2e-стенде вложенных selector'ов с настоящим `ConnectionManager`, `-race`; на конфиге репортёра issue #20 не гонялось (тикет закрыт владельцем 2026-09-14 без подтверждения); в поле с `v1.14.0-lx.38` без жалоб, закрыта владельцем 2026-09-24 |
