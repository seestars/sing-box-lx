...
| Статус | D (done) — мерж выполнен (`f56680e0d`); дрейф сабмодулей закрыт перепрививкой на полную апстрим-ленту (`a4da5d10f`, rc.6) после того, как выборочный cherry-pick сломал rc.5; device-прогона (R4) не было; мерж в поле с `v1.14.0-lx.20-rc.5` без жалоб, закрыта владельцем 2026-09-24 |
| Ветка | `lx` |
