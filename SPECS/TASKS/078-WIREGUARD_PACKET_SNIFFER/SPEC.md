...
| Статус | D (done) — код, юниты, сборка `lx-build`, живой прогон через бинарь; на роутере не гонялось, в поле с `v1.14.0-lx.33` без жалоб, закрыта владельцем 2026-09-24 |
| Ветка | `lx` |
...
- [x] Полевой прогон на роутере: поток WG с LAN-устройства попадает в
  правило `protocol: wireguard` — не проводился; в поле с `v1.14.0-lx.33`
  без жалоб, снят владельцем 2026-09-24.
