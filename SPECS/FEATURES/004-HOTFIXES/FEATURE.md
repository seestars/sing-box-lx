...
библиотеке. Xray эту развилку прошёл давно и подставляет адрес прокси, поэтому
с теми же серверами работает — отсюда и «в v2rayNG всё ок» из отчёта.
...
Урок реестра — про честность диагноза: механизм доказан по первоисточникам
(код sing, документация Go, код Xray, эксперимент), а привязка к конкретному
отчёту — нет, дампа UDP-пути не было. Поэтому в статусе стоит «полевая проверка
открыта», а не «закрыто», и рядом лежит дешёвый способ её закрыть — снять
BND.ADDR с прокси репортёра одним хендшейком. Второй урок — страж на апстрим:
тест, который фиксирует чужой дефект и падает, когда его починят, превращает
условие снятия из строки в реестре в сигнал на бампе зависимости.
