...
памяти — дефолты и строки ошибок те же, что выдаёт текущее ядро.
...
Отдельного ключа нет: версия HTTP выводится из блока `tls` outbound'а по правилу
Xray (`decideHTTPVersion`, SPEC 104). Все режимы работают на всех версиях.

| TLS | Reality | `tls.alpn` | Версия | Провод |
|-----|---------|-----------|--------|--------|
| выкл | — | — | HTTP/1.1 (до SPEC 104 — h2c) | TCP |
| вкл | вкл | любой | HTTP/2 | TCP + Reality |
| вкл | выкл | пусто | HTTP/2 | TCP + TLS, ALPN `h2` |
| вкл | выкл | два и больше элементов | HTTP/2 | TCP + TLS, ALPN как в конфиге |
| вкл | выкл | `["http/1.1"]` | HTTP/1.1 | TCP + TLS |
| вкл | выкл | `["h3"]` | HTTP/3 | UDP + QUIC |
| вкл | выкл | любой другой один элемент | HTTP/2 | TCP + TLS, ALPN как в конфиге |

- Элемент сравнивается точно (`h3`, `http/1.1`).
- **HTTP/3** требует сборки с `with_quic` (у всех поставляемых сборок он есть);
  без него конфиг с `alpn: ["h3"]` не грузится. QUIC-рукопожатие идёт с профилем
  Chrome; `utls.fingerprint` на HTTP/3 не применяется (предупреждение при загрузке,
  конфиг грузится — Xray ведёт себя так же). С `disable_sni` профиль Chrome для
  этого узла выключается (предупреждение). ECH на HTTP/3 не поддержан: конфиг
  грузится, dial узла отвечает ошибкой. Трафик HTTP/3 идёт по UDP через тот же
  `detour`.
- **HTTP/1.1**: долгие запросы (download-GET, потоковый POST) идут каждый по своему
  соединению с `Connection: close`; upload-POST'ы `packet-up` переиспользуют
  keep-alive-соединения.
- **Reality — всегда HTTP/2.** `tls.alpn` без `h2` заменяется на `["h2"]`
  (предупреждение), иначе Reality-сервер с `alpn: ["h3"]` по TCP не поднимется.
- **Без TLS — HTTP/1.1, как у Xray (раньше h2c).** Сервер Xray принимает обе
  формы, поэтому прямые узлы без TLS продолжают работать, а узлы за обратным
  прокси или CDN, который на незашифрованном порту говорит только HTTP/1.1,
  начинают работать. Мультиплексирования нет: каждый поток даунлинка и каждый
  потоковый запрос идут отдельным TCP-соединением.
- Не поддержаны: `downloadSettings` Xray, `finalmask.quicParams` (QUIC работает
  с дефолтами Xray, контроль перегрузки — Cubic, у Xray BBR).
...
> дашед-UUID (`8f14e45f-ceea-4d31-9d4f-d0b8e5c1a2b7`), то же, что шлёт и
...
| `xmux.h_keep_alive_period` | int (секунды) | `0` = деф | Период keep-alive. HTTP/2: период ping (`ReadIdleTimeout` транспорта), `0` = без пингов. HTTP/3: QUIC keep-alive, `0` = 10 с. На HTTP/1.1 не действует. Отрицательное отключает keep-alive. Простое целое, **не** диапазон — как в референсной реализации |
...
Сервер слушает XHTTP только по QUIC (Xray `tlsSettings.alpn: ["h3"]`).
...
```jsonc
{
  ...
  "tls": {
    ...
    "server_name": "example.com",
    "alpn": ["h3"],
    "certificate_public_key_sha256": ["<base64-sha256-ключа-сервера>"]
  },
  "transport": {
    "type": "xhttp",
    "mode": "stream-up",
...
`protocol_version: "3.1"` экспортирует этот набор.
...
`*ecdsa.PublicKey` (P-256) — тот формат, что отдаёт WARP-регистрация на
...
> MASQUE-endpoint в ClientHello — то, на что фильтрует DPI; нейтральный
...
У `service_name` **две** формы, как у Xray. Какая из них — решает один
символ: ведущий `/`.
...
живого Xray пока не прогонялось** — путь на проводе сверен с исходниками Xray и
grpc-go. Доступно с `v1.14.1-lx.8`.
