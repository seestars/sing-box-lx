...
- [Что узнаёт форк](#что-узнаёт-форк)
- [Порядок по умолчанию](#порядок-по-умолчанию)
- [Главное, что нужно знать: считается только первый пакет](#главное-что-нужно-знать-считается-только-первый-пакет)
- [Пример для роутера](#пример-для-роутера)
- [Намеренно не распознаётся](#намеренно-не-распознаётся)
